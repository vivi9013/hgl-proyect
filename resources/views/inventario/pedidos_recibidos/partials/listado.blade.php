<div class="container-fluid py-4">
    ...
    <div class="d-flex gap-2 mb-2 flex-wrap">
        <a href="{{ route('pedidos_recibidos.index') }}"
           class="btn btn-sm {{ $tipo === 'pendiente' ? 'btn-primary' : 'btn-outline-dark bg-white' }} py-2 px-3 fw-bold shadow-sm"
           style="border: 1.5px solid #000; border-radius: 8px;">
            <i class="fa fa-hourglass-half me-1 {{ $tipo === 'pendiente' ? '' : 'text-dark' }}"></i>Pendientes
        </a>
        <a href="{{ route('pedidos_recibidos.aceptados') }}"
           class="btn btn-sm {{ $tipo === 'aceptado' ? 'btn-primary' : 'btn-outline-dark bg-white' }} py-2 px-3 fw-bold shadow-sm"
           style="border: 1.5px solid #000; border-radius: 8px;">
            <i class="fa fa-check-circle me-1 {{ $tipo === 'aceptado' ? '' : 'text-dark' }}"></i>Aceptados
        </a>
        <a href="{{ route('pedidos_recibidos.cancelados') }}"
           class="btn btn-sm {{ $tipo === 'cancelado' ? 'btn-primary' : 'btn-outline-dark bg-white' }} py-2 px-3 fw-bold shadow-sm"
           style="border: 1.5px solid #000; border-radius: 8px;">
            <i class="fa fa-ban me-1 {{ $tipo === 'cancelado' ? '' : 'text-dark' }}"></i>Cancelados
        </a>
    </div>
    ...
    @php
        $routeActual = match($tipo) {
            'aceptado'  => route('pedidos_recibidos.aceptados'),
            'cancelado' => route('pedidos_recibidos.cancelados'),
    ...
    <div class="row mb-4 align-items-end g-3">
        <div class="col-12">
            <form method="GET" action="{{ $routeActual }}" id="formBuscar">
                <div class="row g-2 align-items-end">
                    <div class="col-12 col-md-6 position-relative">
                        ...
                        <div class="input-group" style="border: 1.5px solid #000; border-radius: 10px; overflow: hidden;">
                            ...
                            @if($buscar)
                                <a href="{{ $routeActual }}" class="input-group-text bg-light border-0 text-decoration-none" title="Limpiar">
                    ...
                    <div class="col-6 col-md-3">
                        ...
                        <div class="input-group">
                            ...
                            @if($buscar || $fechaInit || $fechaFin)
                                <a href="{{ $routeActual }}" class="btn btn-outline-secondary" title="Limpiar Filtros">
                ...
                <div class="card-header bg-white border-0 pt-4 px-0 pb-2 d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center gap-3">
                        <h5 class="card-title mb-0 fw-bold text-dark">
                            @if($tipo === 'pendiente')
                                <i class="fa fa-clock-o text-secondary me-2"></i>Pedidos Pendientes por Surtir
                            @elseif($tipo === 'aceptado')
                                <i class="fa fa-check-circle text-success me-2"></i>Pedidos Surtidos / Aceptados
                            @else
                                <i class="fa fa-ban text-danger me-2"></i>Pedidos Cancelados
                            @endif
                        </h5>
                ...
                <div class="card-body p-0 mt-2">
                    <div class="table-responsive">
                        <table id="tablaPedidos" class="table table-hover align-middle mb-0">
                            <thead class="table-light text-uppercase font-size-xs text-secondary letter-spacing-1" style="font-size: 0.75rem;">
                                <tr>
                                    ...
                                    @else
                                        <th>Solicitante</th>
                                        <th class="text-center pe-4" style="width: 100px;">Detalle</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($pedidos as $pedido)
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $folioBg = match($tipo) {
                                                    'cancelado' => 'background-color: #fef2f2; border: 1px solid #fecaca; color: #b91c1c;',
                                                    default     => 'background-color: #eff6ff; border: 1px solid #bfdbfe; color: #1d4ed8;',
                                                };
                                            @endphp
                                            <span style="font-family: Arial, sans-serif; font-size: 0.82rem; font-weight: 600; border-radius: 6px; padding: 3px 10px; display: inline-block; {{ $folioBg }}">
                                                {{ $pedido->folio }}
                                            </span>
                                        </td>
                                        ...
                                        @if($tipo === 'pendiente')
                                            ...
                                            <td class="text-center pe-4">
                                                <a href="{{ route('pedidos_recibidos.detalle', $pedido->id_pedido) }}"
                                                   class="btn btn-sm btn-primary py-1 px-3 rounded-pill fw-bold shadow-sm"
                                                   title="Surtir y liberar pedido" style="font-size: 0.8rem;">
                                                    <i class="fa fa-pencil me-1"></i>Surtir
                                                </a>
                                            </td>
                                        @elseif($tipo === 'aceptado')
                                        ...
                                        @else
                                            ...
                                            <td class="text-center pe-4">
                                                <a href="{{ route('pedidos_recibidos.detalle', $pedido->id_pedido) }}"
                                                   class="btn btn-sm btn-outline-dark py-1 px-2" title="Ver detalle">
                                                    <i class="fa fa-eye"></i>
                                                </a>
                                            </td>
                                        @endif
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ $tipo === 'aceptado' ? '9' : '8' }}" class="text-center text-muted py-4">
                ...
                @if($pedidos->total() > 0)
                    <div class="card-footer bg-white border-0 py-3 px-0 d-flex justify-content-between align-items-center border-top mt-2">
                        ...
                        <nav aria-label="Paginación de pedidos">
                            {{ $pedidos->appends(request()->except('page'))->links('pagination::bootstrap-5') }}
                        </nav>
                    </div>
                @endif
            </div>
