/**
 * Lógica Javascript para el módulo de Categoría de Módulos (JSON AJAX Render)
 */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function cargarPagina(numeroPagina = 1) {
        ...
        fetch(`/categoria-modulos?buscar=${encodeURIComponent(buscar)}&page=${numeroPagina}`, {
            headers: { 
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json'
            }
    ...
    // 4. Configurar listeners de status y colapsar
    function enlazarEventosAcciones() {
        ...
        toggleStatusLinks.forEach(link => {
            ...
            nuevoLink.addEventListener('click', function (e) {
                ...
                const url = `/categoria-modulos/${id}/status`;
        ...
        // Colapsar Toggle
        const toggleColapsarLinks = document.querySelectorAll('.btn-toggle-colapsar');
        toggleColapsarLinks.forEach(link => {
            const nuevoLink = link.cloneNode(true);
            link.parentNode.replaceChild(nuevoLink, link);
            ...
            nuevoLink.addEventListener('click', function (e) {
                e.preventDefault();
                const id = this.getAttribute('data-id');
                const url = `/categoria-modulos/${id}/colapsar`;
                ...
                .catch(err => {
                    console.error('Error al colapsar:', err);
                });
            });
        });
    }
