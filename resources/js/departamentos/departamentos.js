document.addEventListener('DOMContentLoaded', function () {
    ...
    function cargarPagina(numeroPagina = 1) {
        ...
        fetch(`/departamentos?${queryParams.toString()}`, {
        ...
        .catch(err => {
            tbody.style.opacity = '1';
            console.error('Error al cargar el módulo de departamentos:', err);
        });
    }
    ...
    // ─────────────────────────────────────────────────────────
    // LÓGICA: VALIDACIÓN EN TIEMPO REAL DEL NOMBRE Y ABREVIATURA (AJAX)
    // ─────────────────────────────────────────────────────────
    ...
    function verificarDisponibilidad() {
        ...
        verificarTimeout = setTimeout(() => {
            ...
            fetch(`/departamentos/verificar?${params.toString()}`, {
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            })
            .then(r => r.json())
            .then(data => {
                ...
                if (nombre && nombreFeedback) {
                    ...
                    if (data.nombre_disponible) {
                        ...
                        nombreFeedback.textContent = '✔ El nombre está disponible.';
                    } else {
                        nombreFeedback.classList.add('text-danger');
                        nombreFeedback.textContent = '✘ Este departamento ya existe.';
                        bloquear = true;
                    }
                }

                // Abreviatura
                if (abreviatura && abreviaturaFeedback) {
                    ...
                    if (data.abreviatura_disponible) {
                        ...
                        abreviaturaFeedback.textContent = '✘ Esta abreviatura ya existe.';
                        bloquear = true;
                    }
                }
            ...
            .catch(() => {
                if (nombreFeedback)     nombreFeedback.classList.add('d-none');
                if (abreviaturaFeedback) abreviaturaFeedback.classList.add('d-none');
            });
        }, 400);
    }
        ...
        toggleStatusLinks.forEach(link => {
            ...
            newLink.addEventListener('click', function (e) {
                ...
                if (typeof Swal !== 'undefined') {
                    Swal.fire({
                        title:             `¿Desea ${accion} el departamento?`,
                        text:              `El departamento "${nombre}" cambiará de estado en el sistema.`,
                        icon:              iconType,
                    ...
                    if (confirm(`¿Está seguro de que desea ${accion} el departamento "${nombre}"?`)) {
                        doRequest();
                    }
                }
            });
    ...
    if (pageErrors && pageErrors.dataset.errors === '1') {
        const modalEl = document.getElementById('modalRegistrarDepartamento');
    ...
    const canvasBarMob       = document.getElementById('barMobiliarioChart');
    ...
    // Bar Charts
    crearBarChart(canvasBarMob,  'Cantidad de Mobiliario',   '#3b82f6');
